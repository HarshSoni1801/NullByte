...
// Constants (same as create form)
const difficultyOptions = ['easy', 'medium', 'hard'];
const tagOptions = ['array', 'linkedList', 'tree', 'graph', 'dynamicProgramming', 'greedy', 'backtracking', 'sorting', 'searching','twoPointer','stack','string'];
...
// NEW: Validation Error Modal Component (same as create form)
...
function UpdateProblemForm() {
  ...
  const id = p_id; // Use p_id from params
  ...
  // Helper components (same as create form)
  ...
  // Field arrays for dynamic sections (same as create form)
  const {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        setIsLoading(true);
        const response = await axiosClient.get(`/problem/fetch/${id}`);
        const problemData = response.data;
        console.log("response in update Problem file: ", response);
        ...
        reset({
          ...
          referenceSol: problemData.referenceSol,
          judgeWrapperCode: problemData.judgeWrapperCode || [{ language: 'JavaScript', Code: '' }],
        });
  ...
  const onSubmit = async(data) => {
    try {
      ...
      const result = await axiosClient.put(`/problem/update/${id}`, data);
  ...
  // Handle tag changes (same as create form)
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-zinc-900 text-zinc-300 font-sans p-4 sm:p-6 md:p-8 flex items-center justify-center" >
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-600 mx-auto"></div>
          <p className="mt-4 text-zinc-400">Loading problem data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-zinc-900 text-zinc-300 font-sans p-4 sm:p-6 md:p-8" style={{"padding-top": "8rem"}}>
        <div className="max-w-5xl mx-auto">
          <header className="mb-8 text-center">
            <h1 className="text-4xl font-bold text-zinc-100">Update Coding Problem</h1>
            <p className="text-zinc-400 mt-2">Edit the problem details, test cases, and solutions.</p>
          </header>
          <main>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="flex justify-between mt-8">
                <button 
                  ...
                  className="btn bg-red-800 hover:bg-red-700 text-white border-none text-lg px-8 py-3"
                >
                  Delete Problem
                </button>
                <div className="flex gap-4">
                  <button 
                    ...
                    className="btn bg-zinc-600 hover:bg-zinc-500 text-white border-none text-lg px-8 py-3"
                  >
                    Cancel
                  </button>
                  <button 
